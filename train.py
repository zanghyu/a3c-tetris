...
parser.add_argument('-l', '--log-dir', type=str, default="model",
...
def create_commands(session, num_workers, logdir, shell='bash',
                    mode='tmux', psPort=12222,
                    delay=0, pretrain=None):
...
def run():
    ...
    psPort = 12200+ 10*args.expId
    delay = 5*args.expId

    cmds, notes = create_commands(args.expName, args.num_workers, 
                                    args.log_dir, mode=args.mode, psPort=psPort,
                                    delay=delay, pretrain=args.pretrain)
    if args.dry_run:
