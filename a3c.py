...
def process_rollout(rollout, gamma, lambda_=1.0, clip=False):
    ...
    return Batch(batch_si, batch_a, batch_adv, batch_r, rollout.terminal, rollout.tot_cleared)

Batch = namedtuple("Batch", ["si", "a", "adv", "r", "terminal", "tot_cleared"])
...
class PartialRollout(object):
    ...
    def add(self, state, action, reward, value, terminal, tot_cleared,
...
def env_runner(gameState, policy, num_local_steps, summary_writer, predictor):
    ...
    while True:
        ...
        # policy.sample, policy.vf, policy.state_out
            ...
            act = [0]*ACTION_SIZE
            act[stepAct] = 1
            state, reward, terminal, cleared = gameState.frame_step(act)
            ...
            curr_tuple = [last_state, action, reward, value_, terminal, tot_cleared]
